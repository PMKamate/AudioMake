...
import com.demo.audiomake.apiResponse.Entry
...
class EntryDB(
    ...
    companion object {
        @JvmField
        val CREATOR: Parcelable.Creator<EntryDB> = object : Parcelable.Creator<EntryDB> {
            override fun createFromParcel(source: Parcel): EntryDB = EntryDB(source)
            override fun newArray(size: Int): Array<EntryDB?> = arrayOfNulls(size)
        }
    }
}

//extension function to covert model to DTO and change image path
fun Entry.mapDto() = EntryDB(
